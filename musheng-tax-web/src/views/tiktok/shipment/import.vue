  ...
  <div class="page-container">
    ...
    <a-card>
      ...
      <a-upload-dragger :custom-request="handleUpload" :show-upload-list="false" accept=".xlsx,.xls" :disabled="!siteCode">
        ...
        <p class="ant-upload-text">点击或拖拽上传 FBT 货件整合版 Excel</p>
        <p class="ant-upload-hint">需包含"货件详情"Sheet页</p>
      </a-upload-dragger>
      ...
      <a-spin :spinning="loading" style="margin-top: 16px">
        <a-result v-if="result" status="success" :title="`导入完成：${result.shipments} 个货件，${result.items} 条明细`">
          <template #extra>
            <a-button type="primary" @click="$router.push('/tiktok/shipment/list')">查看货件列表</a-button>
          </template>
        </a-result>
      </a-spin>
    </a-card>
...
import { importTiktokShipment } from '@/api/tiktok'
import { useTiktokSites } from '@/composables/tiktok/useTiktokSites'
import { message } from 'ant-design-vue'

const { sites } = useTiktokSites()
const siteCode = ref('')
const result = ref<{ shipments: number; items: number } | null>(null)
...
async function handleUpload(options: any) {
  ...
  loading.value = true
  try {
    const res: any = await importTiktokShipment(file, siteCode.value)
